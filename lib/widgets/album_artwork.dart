...
class AlbumArtwork extends StatelessWidget {
  ...
  Widget _buildContent(
    ...
    if (preserveAspectRatio) {
      return Container(
        ...
        child: ClipRRect(
          borderRadius: BorderRadius.circular(resolvedRadius),
          child: _buildImageNatural(isDark, cacheSize, validSize),
        ),
      );
    }
    ...
    return Container(
      ...
      child: ClipRRect(
        borderRadius: BorderRadius.circular(resolvedRadius),
        child: _buildImage(isDark, cacheSize, validSize),
      ),
    );
  }

  Widget _buildImageNatural(bool isDark, int cacheSize, double fallbackSize) {
    if (coverArt == null || coverArt!.isEmpty) return _buildPlaceholder(isDark, fallbackSize);
    ...
    if (isLocalFilePath(coverArt)) {
      ...
      return Image.file(
        ...
        fit: BoxFit.contain,
        errorBuilder: (ctx, err, stack) => _buildPlaceholder(isDark, fallbackSize),
      );
    }
    ...
    return Builder(
      builder: (context) {
        ...
        if (imageUrl.isEmpty) return _buildPlaceholder(isDark, fallbackSize);
        return CachedNetworkImage(
          ...
          placeholder: (ctx, url) => _buildPlaceholder(isDark, fallbackSize),
          errorWidget: (ctx, err, stack) {
            debugPrint('AlbumArtwork error (natural): $err');
            return _buildNetworkImageFallback(imageUrl, isDark, BoxFit.contain, fallbackSize);
          },
        );
      },
    );
  ...
  Widget _buildImage(bool isDark, int cacheSize, double fallbackSize) {
    if (coverArt == null || coverArt!.isEmpty) return _buildPlaceholder(isDark, fallbackSize);
      ...
      builder: (context) {
        ...
        return CachedNetworkImage(
          ...
          errorWidget: (ctx, err, stack) {
            ...
            return _buildNetworkImageFallback(imageUrl, isDark, BoxFit.cover, fallbackSize);
          },
        );
      },
    );
  ...
  Widget _buildNetworkImageFallback(String url, bool isDark, BoxFit fit, double fallbackSize) {
    return Image.network(
      url,
      fit: fit,
      errorBuilder: (ctx, err, stack) {
        debugPrint('Network image fallback error: $err');
        return _buildPlaceholder(isDark, fallbackSize);
      },
    );
  }

  Widget _buildPlaceholder(bool isDark, double fallbackSize) {
    return Container(
      width: fallbackSize,
      height: fallbackSize,
      decoration: BoxDecoration(
      ...
      child: Center(
        child: Icon(
          ...
          size: (fallbackSize / 3).clamp(16.0, 60.0),
