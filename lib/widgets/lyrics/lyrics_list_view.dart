...
class _LyricsListViewState extends State<LyricsListView> {
  ...
  Widget build(BuildContext context) {
    final hasNoLyrics = widget.lyrics.isEmpty ||
        (widget.lyrics.length == 1 &&
            widget.lyrics.first.text.toLowerCase().contains('testo non disponible'));

    if (hasNoLyrics) {
      final isFr = Localizations.localeOf(context).languageCode == 'fr';
      return Center(
        child: Text(
          isFr ? 'Paroles non disponibles' : 'No lyrics available',
