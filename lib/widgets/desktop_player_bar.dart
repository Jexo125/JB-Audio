  ...
  Widget build(BuildContext context) {
    ...
    return Selector<PlayerProvider, (Duration, Duration)>(
      ...
      builder: (context, data, _) {
        ...
        final isRecentlySeeked = _lastSeekTime != null &&
            DateTime.now().difference(_lastSeekTime!) < const Duration(milliseconds: 500);

        final currentMs = _isDragging
            ? _dragValue
            : (isRecentlySeeked ? _dragValue : position.inMilliseconds.toDouble());

        return SizedBox(
          width: 400,
          child: Row(
            children: [
              Text(_formatDuration(Duration(milliseconds: currentMs.toInt())), style: timeStyle),
              const SizedBox(width: 8),
              Expanded(
                child: SizedBox(
                  height: 20,
                  child: SliderTheme(
                    ...
                    child: Slider(
                      ...
                      min: 0.0,
                      max: duration.inMilliseconds.toDouble().clamp(1.0, double.maxFinite),
                      onChangeStart: (v) {
                        setState(() {
                          _isDragging = true;
                          _dragValue = v;
