...
class _PlaybackProgressSliderState extends State<PlaybackProgressSlider> {
  ...
  // Local "source of truth" during interaction
  Duration? _localValue;
  ...
  Widget build(BuildContext context) {
    ...
    // When dragging, use drag value. 
    // When just finished dragging, keep using local value for a moment to avoid jumps.
    final displayMs = _isDragging 
        ? _dragValue 
        : (_localValue?.inMilliseconds.toDouble() ?? widget.position.inMilliseconds.toDouble());
        
    final safeMax = maxDuration > 0 ? maxDuration : 1.0;
    final progress = (displayMs / safeMax).clamp(0.0, 1.0);
    ...
    return Column(
      ...
      children: [
        GestureDetector(
          ...
          onHorizontalDragStart: (details) {
            setState(() {
              ...
              _localValue = Duration(milliseconds: _dragValue.toInt());
            });
          },
          ...
          onHorizontalDragEnd: (details) {
            final seekPos = Duration(milliseconds: _dragValue.toInt());
            setState(() {
              _isDragging = false;
              _localValue = seekPos;
            });
            widget.onChanged(seekPos);
            
            // Clear local value after a delay once engine should have caught up
          ...
          onTapDown: (details) {
            ...
            final seekPos = Duration(milliseconds: tapValue.toInt());
        ...
        Row(
          ...
          children: [
            Text(
              _formatDuration(Duration(milliseconds: displayMs.toInt())),
              style: TextStyle(
            ...
            Text(
              "-${_formatDuration(widget.duration - Duration(milliseconds: displayMs.toInt()))}",
              style: TextStyle(
                color: Colors.white.withValues(alpha: 0.7),
                fontSize: 12,
                fontWeight: FontWeight.w500,
