...
class Song {
  ...
  /// Retourne une chaîne formatée contenant les infos audio (ex: "FLAC • 1411 kbps")
  String get audioInfo {
    final List<String> parts = [];

    // 1. Extension / Format (ex: FLAC, MP3)
    if (suffix != null && suffix!.isNotEmpty) {
      parts.add(suffix!.toUpperCase());
    }

    // 2. Débit / Bitrate (ex: 320 kbps)
    if (bitRate != null && bitRate! > 0) {
      final kbps = bitRate! > 5000 ? (bitRate! / 1000).round() : bitRate;
      parts.add('$kbps kbps');
    }

    // 3. Optionnel : Dolby Atmos
    if (hasDolbyAtmos == true) {
