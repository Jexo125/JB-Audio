...
class LibraryProvider extends ChangeNotifier {
  ...
  Future<SearchResult> search(String query) async {
    ...
    if (cleanQuery.isEmpty) return SearchResult();

    // 1. Recherche locale (toujours effectuée si on a des données cachées)
    final localResult = _searchLocal(cleanQuery);

    if (_localOnlyMode) {
      return localResult;
    }

    // 2. Recherche serveur pour compléter
    try {
      final serverResult = await _subsonicService.search(cleanQuery);

      // 3. Fusion et dédoublonnage
      final mergedSongs = <String, Song>{};
      final mergedAlbums = <String, Album>{};
      final mergedArtists = <String, Artist>{};

      // On donne la priorité aux résultats serveurs pour les métadonnées fraîches
      for (final s in serverResult.songs) {
        mergedSongs[s.id] = s;
      }
      for (final a in serverResult.albums) {
        mergedAlbums[a.id] = a;
      }
      for (final ar in serverResult.artists) {
        mergedArtists[ar.id] = ar;
      }

      // On ajoute les résultats locaux non présents
      for (final s in localResult.songs) {
        if (!mergedSongs.containsKey(s.id)) mergedSongs[s.id] = s;
      }
      for (final a in localResult.albums) {
        if (!mergedAlbums.containsKey(a.id)) mergedAlbums[a.id] = a;
      }
      for (final ar in localResult.artists) {
        if (!mergedArtists.containsKey(ar.id)) mergedArtists[ar.id] = ar;
      }

      // 4. Re-tri par pertinence intelligente
      final songList = mergedSongs.values.toList();
      songList.sort((a, b) => _calculateSongScore(cleanQuery, b)
          .compareTo(_calculateSongScore(cleanQuery, a)));

      final albumList = mergedAlbums.values.toList();
      albumList.sort((a, b) => _calculateAlbumScore(cleanQuery, b)
          .compareTo(_calculateAlbumScore(cleanQuery, a)));

      final artistList = mergedArtists.values.toList();
      artistList.sort((a, b) => _calculateArtistScore(cleanQuery, b)
          .compareTo(_calculateArtistScore(cleanQuery, a)));

      return SearchResult(
        songs: songList.take(50).toList(),
        albums: albumList.take(20).toList(),
        artists: artistList.take(20).toList(),
      );
    } catch (e) {
      debugPrint('Error during hybrid search: $e');
      return localResult;
    }
  }

  double _calculateSongScore(String query, Song song) {
    final titleScore = SearchUtility.calculateScore(query, song.title);
    final artistScore = SearchUtility.calculateScore(query, song.artist ?? '');
    final albumScore = SearchUtility.calculateScore(query, song.album ?? '');
    
    // On privilégie le titre, puis l'artiste, puis l'album
    return titleScore * 1.0 + artistScore * 0.8 + albumScore * 0.5;
  }

  double _calculateAlbumScore(String query, Album album) {
    final nameScore = SearchUtility.calculateScore(query, album.name);
    final artistScore = SearchUtility.calculateScore(query, album.artist ?? '');
    return nameScore * 1.0 + artistScore * 0.7;
  }

  double _calculateArtistScore(String query, Artist artist) {
    return SearchUtility.calculateScore(query, artist.name);
  }
  ...
  SearchResult _searchLocal(String query) {
    ...
    final scoredSongs = <Song, double>{};
    for (final s in _cachedAllSongs) {
      final score = _calculateSongScore(q, s);
      if (score > 0.1) scoredSongs[s] = score;
    }
    final sortedSongs = scoredSongs.keys.toList()
      ..sort((a, b) => scoredSongs[b]!.compareTo(scoredSongs[a]!));

    // Recherche dans les albums
    final scoredAlbums = <Album, double>{};
    for (final a in _cachedAllAlbums) {
      final score = _calculateAlbumScore(q, a);
      if (score > 0.1) scoredAlbums[a] = score;
    }
    final sortedAlbums = scoredAlbums.keys.toList()
      ..sort((a, b) => scoredAlbums[b]!.compareTo(scoredAlbums[a]!));

    // Recherche dans les artistes
    final scoredArtists = <Artist, double>{};
    for (final ar in _artists) {
      final score = _calculateArtistScore(q, ar);
      if (score > 0.1) scoredArtists[ar] = score;
    }
    final sortedArtists = scoredArtists.keys.toList()
      ..sort((a, b) => scoredArtists[b]!.compareTo(scoredArtists[a]!));

    return SearchResult(
      songs: sortedSongs.take(50).toList(),
      artists: sortedArtists.take(20).toList(),
      albums: sortedAlbums.take(20).toList(),
    );
  }
